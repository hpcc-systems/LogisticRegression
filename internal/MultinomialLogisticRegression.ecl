...
/**
  ...
  * @param alpha    learning rate with a default value 0.1.
  * @param tol      converge threshold based on the average loss. The default value is 0.001.
  *
  */
EXPORT MultinomialLogisticRegression( UNSIGNED4 max_iter = 50,
  ...
  wis1:= TABLE(dep, {wi, cls := MAX(GROUP, value)}, wi, MERGE);
  wis := JOIN(wis0, wis1,
             LEFT.wi = RIGHT.wi,
             TRANSFORM(l_wi,
                      d := LEFT.dim;
